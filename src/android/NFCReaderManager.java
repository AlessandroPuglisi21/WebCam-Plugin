...
public class NFCReaderManager {
    ...
    // Costanti per i tipi di carta (dal MainActivity)
    ...
    public boolean initReader(int portType, String devicePath, int baudRate) {
        try {
            ...
            Log.e(TAG, "Eccezione durante l'inizializzazione", e);
            return false;
        }
    }
    ...
    /**
     * Identifica automaticamente una carta NFC
     */
    public JSONObject identifyCard() {
        try {
            ...
            // Se non trova TypeA, prova TypeB
            result = call_comPro.lc_findTypeB(deviceHandle, (byte)0, snr, snrSize);
            if (result == 0) {
                JSONObject cardInfo = new JSONObject();
            ...
            // Prova ISO15693
            result = call_comPro.lc_find15693(deviceHandle, (byte)0, snr, snrSize);
            if (result == 0) {
                JSONObject cardInfo = new JSONObject();
    ...
    public JSONObject readUHF() {
        try {
            ...
            uhfInfo.put("message", "Nessun tag UHF rilevato");
